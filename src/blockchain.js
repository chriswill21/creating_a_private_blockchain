    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                // Hash new block
                let newBlockHash = SHA256(JSON.stringify(block)).toString()

                block.hash = newBlockHash

                // Update chain
                self.height = self.height + 1
                self.chain.push(block)

                await self.validateChain().then((errs) => {
                    if (errs.length > 0) {
                        reject(new Error('Failed to add new block to the blockchain.'))
                    }
                })
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (validTime && verifiedMessage) {
                ...
                reject(new Error('Failed to submit start because message wasn\'t validated in time.'))
            }
        });
    }
